...
class NGCF(nn.Module):
    ...
    def forward(self, users, pos_items, neg_items, drop_flag=True):
        ...
        for k in range(len(self.layers)):
            ...
            # transformed sum messages of neighbors.
            sum_personality_embeddings = torch.matmul(side_personality_embeddings, self.weight_dict['W_gc_%d' % k]) + self.weight_dict['b_gc_%d' % k]

            # bi messages of neighbors.
            # element-wise product
            bi_personality_embeddings = torch.mul(ego_personality_embeddings, side_personality_embeddings)
            # transformed bi messages of neighbors.
            bi_personality_embeddings = torch.matmul(bi_personality_embeddings, self.weight_dict['W_bi_%d' % k]) + self.weight_dict['b_bi_%d' % k]

            # non-linear activation.
            ego_personality_embeddings = nn.LeakyReLU(negative_slope=0.2)(sum_personality_embeddings + bi_personality_embeddings)

            # message dropout.
            ego_personality_embeddings = nn.Dropout(self.mess_dropout[k])(ego_personality_embeddings)

            # normalize the distribution of embeddings.
            norm_personality_embeddings = F.normalize(ego_personality_embeddings, p=2, dim=1)

            all_personality_embeddings += [norm_personality_embeddings]
